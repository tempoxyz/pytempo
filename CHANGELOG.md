...
- Update dependencies. Breaking change: Python 3.10 or newer is now required. (by @DerekCofausper, [#105](https://github.com/tempoxyz/pytempo/pull/105))
...
- Updated pinned GitHub Actions SHA references to the latest `tempoxyz/gh-actions` commit (`d39baa2b`) across all CI/CD workflows. Added `.pinact.yaml` to exempt the `tempoxyz/changelogs` action from tag verification. (by @ShanedaSilva, [#103](https://github.com/tempoxyz/pytempo/pull/103))
- Replaced the hardcoded `COUNTER_CONTRACT` address with a dynamically deployed `counter_contract` fixture that deploys an isolated counter contract at test time, and introduced a `TEST_ADDRESS` placeholder for offline-only tests. Added storage slot assertions to batch transaction tests to verify call counts. (by @ShanedaSilva, [#102](https://github.com/tempoxyz/pytempo/pull/102))
- Restored live integration coverage for direct AccountKeychain authorization selectors, replacing the TIP-1099 transaction-embedded authorization approach with direct `authorizeKey` precompile selector calls. (by @DerekCofausper, [#89](https://github.com/tempoxyz/pytempo/pull/89))
- Restored live integration coverage for direct AccountKeychain authorization selectors. (by @DerekCofausper, [#89](https://github.com/tempoxyz/pytempo/pull/89))
